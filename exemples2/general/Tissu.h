...
class Tissu : public Dessinable {
	...
	explicit Tissu(Masse*, SupportADessin* = nullptr);
    ...
    std::vector<Masse*> check_contrainte(Contrainte*) const;
    
    void couper(Vecteur3D centre, double R);
	
	friend class TissuCompose;
...
class TissuRectangle : public Tissu {
	...
	void accroche(unsigned short int i = 2);				//pour immobiliser une coté
