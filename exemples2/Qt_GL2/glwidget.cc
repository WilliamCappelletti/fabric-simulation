...
void GLWidget::keyPressEvent(QKeyEvent* event)
{
  ...
  case Qt::Key_P:
	c.augmente_compteur_graphique();
	break;
  
  
  case Qt::Key_I:
	c.augmente_compteur_numerique();
	break;
...
void GLWidget::timerEvent(QTimerEvent* event)
{
  ...
  double dt = chronometre.restart()/(12000.0*c.coefficient());
...
void GLWidget::ecrire() const 
{
    label->setFrameStyle(QFrame::Panel | QFrame::Sunken);

    QString nom = QString::fromStdString(c.nom_integrateur());
    label->setText(nom);
